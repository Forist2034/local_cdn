let package = import ./cache-proxy;
in {
  inherit package;
  module = { cert, ... }:
    { config, lib, pkgs, ... }: {
        ...
        local_cdn.proxy = {
          ...
          servers = mkOption {
            type = types.attrsOf (types.submodule {
              options = { cert = cert.mkOption { default_ca = "proxy"; }; };
            });
            default = { };
          };
        };
      ...
      config = let cfg = config.local_cdn.proxy;
      ...
      in {
        systemd.services."local_cdn-proxy@" =
          let bin_drv = pkgs.callPackage package { };
          in {
        ...
        local_cdn.certgen.configs = lib.mkMerge
          (builtins.map (cfg: cfg.cert_config.certgen)
        ...
        services.nginx.virtualHosts = builtins.mapAttrs (domain: cfg: {
          ...
          locations."/" = {
            proxyPass =
              "http://unix:/run/local_cdn/proxy/${domain}/proxy.sock:";
