...
pub mod header_map {
                ...
                if count == 1 {
                    if let Some(v) = self.0.iter().next() {
                        if let Ok(s) = v.to_str() {
                            ...
                            return ser.serialize_bytes(serde_bytes::Bytes::new(v.as_bytes()));
                        }
                    }
                }
                {
                    ...
                    for v in self.0.iter() {
                        match v.to_str() {
                            ...
                            Err(_) => {
                                seq.serialize_element(serde_bytes::Bytes::new(v.as_bytes()))?
                            }
                        }
                    }
                    seq.end()
    ...
    enum OneOrMore<'a> {
        One(Cow<'a, serde_bytes::Bytes>),
        More(Vec<Cow<'a, serde_bytes::Bytes>>),
    }
    ...
    impl<'de> Visitor<'de> for OneOrMoreVisitor {
        ...
        fn visit_borrowed_str<E: de::Error>(self, s: &'de str) -> Result<Self::Value, E> {
            Ok(OneOrMore::One(Cow::Borrowed(serde_bytes::Bytes::new(
                s.as_bytes(),
            ))))
        }

        fn visit_str<E: de::Error>(self, s: &str) -> Result<Self::Value, E> {
            Ok(OneOrMore::One(Cow::Owned(serde_bytes::ByteBuf::from(
        ...
        fn visit_borrowed_bytes<E: de::Error>(self, s: &'de [u8]) -> Result<Self::Value, E> {
            Ok(OneOrMore::One(Cow::Borrowed(serde_bytes::Bytes::new(s))))
        }
        ...
        fn visit_byte_buf<E: de::Error>(self, s: Vec<u8>) -> Result<Self::Value, E> {
            Ok(OneOrMore::One(Cow::Owned(serde_bytes::ByteBuf::from(s))))
        }
    }
    ...
    impl HeaderMapVisitor {
        ...
        fn multi<E: de::Error>(
            ...
            map: &mut HeaderMap,
            key: &str,
            mut vals: Vec<Cow<'_, serde_bytes::Bytes>>,
        ) -> Result<(), E> {
            if vals.len() == 1 {
                return self.single(map, key, vals.remove(0).into_owned().into_vec());
            }
            let key = HeaderName::from_bytes(key.as_bytes())
                .map_err(|_| de::Error::invalid_value(Unexpected::Str(key), self))?;
            for val in vals {
                let val = HeaderValue::try_from(val.into_owned().into_vec())
            ...
            M: MapAccess<'de>,
        {
            ...
            if !self.is_human_readable {
                while let Some((key, arr)) =
                    access.next_entry::<Cow<str>, Vec<Cow<serde_bytes::Bytes>>>()?
                {
                ...
                while let Some((key, val)) = access.next_entry::<Cow<str>, OneOrMore>()? {
                    match val {
                        OneOrMore::One(val) => {
                            self.single(&mut map, &key, val.into_owned().to_vec())?
                        }
...
pub mod version {
    ...
    pub fn serialize<S: Serializer>(version: &Version, ser: S) -> Result<S::Ok, S::Error> {
        ser.serialize_str(if *version == Version::HTTP_10 {
            "HTTP/1.0"
            ...
            return Err(serde::ser::Error::custom("http version"));
        })
    }
